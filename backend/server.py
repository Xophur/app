...
# Fixed price package
QUIZ_PACKAGES = {
    "full_results": 10.00  # $10 for full quiz results
}
...
class QuizAnswers(BaseModel):
    answers: List[int]  # 25 integers from 1-5
...
def compute_scores(answers: List[int]) -> Dict:
    ax = sum(answers[0:4])  # Q1-4
    av = sum(answers[4:8])  # Q5-8
    q13r = 6 - answers[12]  # Reverse score
    q14r = 6 - answers[13]  # Reverse score
...
def compute_teaser_results(answers: List[int]) -> Dict:
    """Return limited results for free tier"""
    scores = compute_scores(answers)
    ax, av, cr, ps = scores["ax"], scores["av"], scores["cr"], scores["ps"]
    attach = attachment_style(ax, av)
...
async def submit_quiz(request: QuizSubmitRequest):
    """Submit quiz and get teaser results (free) with option to unlock full"""
...
async def get_results(result_id: str):
    """Get quiz results - full if paid, teaser if not"""
...
async def create_checkout_session(request: CheckoutRequest, http_request: Request):
    """Create Stripe checkout session for unlocking full results"""
...
async def get_checkout_status(session_id: str, http_request: Request):
    """Check payment status and unlock results if paid"""
    api_key = os.environ.get("STRIPE_API_KEY")
    if not api_key:
        raise HTTPException(status_code=500, detail="Payment not configured")
    ...
    # If paid, unlock the quiz results
...
async def email_results(request: EmailResultsRequest):
    """Email results to user (paid feature - simulated)"""
    result = await db.quiz_results.find_one({"id": request.result_id}, {"_id": 0})
    ...
    # For now, return success and save email intent
    await db.quiz_results.update_one(
